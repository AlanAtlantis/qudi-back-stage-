	window.onload = function() {
				var input = document.getElementById("upgteimg1");
				var showui = document.getElementById("showui1");
				var result;
				var dataArr = []; // 储存所选图片的结果(文件名和base64数据)
				var fd; //FormData方式发送请求
				var showinput = document.getElementById("showinput1");
				...
				function onclickimg() {
					...
					var lilength = document.querySelectorAll('ul#showui1 li');
						...
						lilength[i].getElementsByTagName('img')[0].onclick = function(num) {
							return function() {
								if(num == 0) {} else {
									...
									var lists = $("ul#showui1 li").length;
									for(var j = 0; j < lists; j++) {
										var usid = $("ul#showui1 li")[j].getElementsByTagName('img')[3];
