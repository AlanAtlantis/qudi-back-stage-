				...
				<table class="table table-hover">
					...
					<tbody>
						...
						<c:forEach var="item" items="${message.object }" varStatus="idx">
							<tr>
								<th scope="row"><%-- ${item.goodsId } --%>${idx.index }</th>
								...
								<td>
									...
									<form action="sys/goodsList/updatePage">
										<input type="hidden" value="${item.goodsId}" name="goodsId">
										<button type="submit" class="btn btn-info btn-xs">商品修改</button>
									</form>
									<form action="sys/goodsList/uploadPage">
										<input type="hidden" value="${item.goodsId}" name="goodsId">
										<button type="submit" class="btn btn-info btn-xs">上传商品图片</button>
									</form>
								</td>
							</tr>
						</c:forEach>
